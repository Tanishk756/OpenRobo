...
> [!IMPORTANT]
> Milestone 7.2.2 focuses purely on orchestration, scheduling, and canary stages. Automatic runtime health gates and automatic rollback triggers belong to Milestone 7.2.3.
...
- \DeploymentRequest\:
  ...
  - \rtifact_url\: Trusted HTTPS origin URL
  - \	arget_filter\: Platform/architecture/tag selector
  - ollout_strategy\: \CANARY_STAGED\ | \IMMEDIATE\ | \SINGLE_DEVICE  - \canary_stages\: List of percentage thresholds and soaking periods (e.g. \[10%, 50%, 100%]\)
- \DeviceDeploymentState\:
  - \PENDING\ -> \DOWNLOADING\ -> \VERIFYING\ -> \STAGING\ -> \STAGED\ -> \ACTIVATING\ -> \ACTIVE\ | \FAILED
### 2. Trusted Artifact Distribution
...
### 3. Progressive Canary Engine
- Device partitioning by cohort / tags / random sampling.
- Progression gates contingent on stage completion without failure.
...
1. **Deployment Core Models (\openrobo_fleet.deployment\ or \pps/api/services/deployment_service.py\)**:
...
4. **Canary Rollout Coordinator**:
   - Progressive batch scheduler with configurable soak intervals.

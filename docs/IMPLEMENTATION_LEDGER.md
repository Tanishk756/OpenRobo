...
| **REQ-M7-29** | Release Signing Gate & Explicit Dev Keypair Generation | VERIFIED | openrobo release build requires explicit signing key; dev key generation requires OPENROBO_DEV_RELEASE_SIGNING=true + --dev | test_signing.py, test_release_cli.py | openrobo_release/signing.py, openrobo_cli/release.py | None | M7.2.1.1 Security |
| **REQ-M7-30** | Agent TrustedReleaseKeyStore & Fail-Closed Metadata | VERIFIED | TrustedReleaseKeyStore local key management by key_id; unparseable metadata fails closed with KEY_METADATA_INVALID; revoked keys rejected | test_trust_store.py, test_signing.py | openrobo_release/trust_store.py, openrobo_release/verification.py | None | M7.2.1.1 Security |
| **REQ-M7-31** | Link-Free Safe Extraction, Canonical Collision & Quarantine | VERIFIED | Links forbidden by default; canonical alias & Windows reserved name rejection; isolated quarantine staging | test_extraction.py | openrobo_release/extraction.py, openrobo_agent/deployment/staging.py | None | M7.2.1.1 Security |
| **REQ-M7-32** | Crash-Consistent Activation Journal & Startup Reconciliation | VERIFIED | activation.intent.json atomic transaction record; ABSlotManager reconciles pointer and slot state on startup | test_slots.py | openrobo_agent/deployment/activation.py, openrobo_agent/deployment/slots.py | None | M7.2.1.1 Core |
| **REQ-M7-33** | Rollback Cryptographic Re-Verification & Revocation Safety | VERIFIED | Rollback re-evaluates manifest signature, trust store key status, and full file hashes before pointer flip; blocks revoked keys and tampered files | test_slots.py, test_local_ota_deployment.py | openrobo_agent/deployment/rollback.py | None | M7.2.1.1 Security |
| **REQ-M7-34** | Zero Implicit Safety Defaults & Telemetry Freshness | VERIFIED | Policy returns POLICY_CONFIGURATION_INVALID if unconfigured; evaluates telemetry freshness max_age_seconds returning STALE_TELEMETRY | test_policy.py | openrobo_release/policy.py | None | M7.2.1.1 Core |
| **REQ-M7-35** | Secret Exclusion Reporting & Heuristic Transparency | VERIFIED | High-confidence secret files denied; heuristic warnings emitted in ExclusionReport; legitimate source files like tokenizer.py preserved | test_archive.py | openrobo_release/archive.py | None | M7.2.1.1 Core |

## Milestone 7.2.1.2 — Final Release Trust, Evidence Consistency & Remote-OTA Readiness Closure

| Requirement ID | Requirement Summary | Status | Implementation Details | Test Coverage | Source Files | Known Limitations | Downstream Dependencies |
|---|---|---|---|---|---|---|---|
| **REQ-M7-36** | Development Key Override Triple AND-Gate | `VERIFIED` | `stage_release_artifact` strictly requires `ENVIRONMENT=development` AND `OPENROBO_ALLOW_DEV_RELEASE_KEY=true` AND `allow_dev_key=True` | `test_slots.py` | `openrobo_agent/deployment/staging.py` | None | Staging security |
| **REQ-M7-37** | Active-Only Key Status & Timestamp Fail-Closed | `VERIFIED` | Rejects `EXPIRED` or `REVOKED` keys; any populated `revoked_at` treats key as revoked; timezone-naive timestamps fail closed | `test_signing.py` | `openrobo_release/verification.py` | None | Trust store security |
| **REQ-M7-38** | Trust Store Key ID Safety & Dynamic Revocation | `VERIFIED` | Restricts `key_id` to `^[A-Za-z0-9._-]{1,64}$`; dynamic UTC revocation timestamps; atomic file persistence | `test_trust_store.py` | `openrobo_release/trust_store.py` | None | Local PKI |
| **REQ-M7-39** | Required ROS Distro Fail-Closed & Architecture Normalization | `VERIFIED` | Staging fails closed with `TARGET_ENVIRONMENT_UNKNOWN` if release requires ROS distro and host is unknown; normalizes `x86_64`/`amd64`, `aarch64`/`arm64` | `test_signing.py`, `test_local_ota_deployment.py` | `openrobo_release/verification.py` | None | Target compatibility |
| **REQ-M7-40** | Safety Telemetry Timestamp Requirement & Skew Tolerance | `VERIFIED` | All enabled safety telemetry requires timestamp evidence envelope; future skew bounded by `max_future_skew_seconds` (default 5.0s) | `test_policy.py` | `openrobo_release/policy.py`, `openrobo_release/models.py` | None | Safety gates |
| **REQ-M7-41** | Distinct Canonical Manifest Digest | `VERIFIED` | Computes `manifest_digest = SHA-256(canonical_manifest_bytes(manifest))`; stores distinct `manifest_digest`, `artifact_digest`, and `workspace_digest` | `test_slots.py` | `openrobo_agent/deployment/staging.py` | None | Slot metadata |
| **REQ-M7-42** | Compressed Archive Size Limit & Canonical Path Mapping | `VERIFIED` | Enforces `max_archive_bytes` (100MB) prior to decompression; extraction maps validated canonical paths; path-aware containment check | `test_extraction.py` | `openrobo_release/extraction.py` | None | Extraction security |

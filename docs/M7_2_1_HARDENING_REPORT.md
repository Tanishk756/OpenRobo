...
**Baseline HEAD:** `ff6ca60f9e2344700046adb45bad86871d6e2156`
**Feature Branch:** `feature/m7-2-1-release-hardening`
...
- **Private Key Validation**: `validate_private_key_file` validates file existence, regular file status (non-symlink), reasonable size bounds (<= 1MB), Ed25519 cryptography, and warns/fails on loose POSIX permissions (> 0600). Private keys are never copied into releases, manifests, or agent states.

### 2.2 Agent Trusted Release Key Store & Fail-Closed Metadata (Phases 4–7)
- **Trusted Key Store (`TrustedReleaseKeyStore`)**: Agents resolve release trust from an agent-owned directory (`/etc/openrobo-agent/trusted-release-keys/` or configured path) keyed by `release_key_id`.
- **No Arbitrary Key Injection**: Deployments cannot self-authenticate by injecting arbitrary public keys via CLI. Development override is gated strictly by both `ENVIRONMENT=development` and `OPENROBO_ALLOW_DEV_RELEASE_KEY=true` along with explicit caller argument.
- **Fail-Closed Metadata**: `ReleaseVerifier` strictly parses ISO 8601 timezone-aware timestamps. Unparseable dates fail with `KEY_METADATA_INVALID`. Never swallows exceptions.
...
- **Links Forbidden by Default**: `SafeArtifactExtractor` sets `allow_symlinks=False` and `allow_hardlinks=False` by default. Symlink pivot attacks and hardlink attacks fail closed.
- **Canonical Path Collision & Alias Rejection**: Member paths are normalized (`a//b`, `a/./b` resolve to canonical `a/b`) and duplicate destination collisions are blocked. Case-insensitive collisions are rejected across all targets as portable-release canonicalization.
- **Control Characters & Device Names**: Rejects NUL and ASCII control characters in paths. Windows reserved device names (`CON`, `PRN`, `AUX`, `NUL`, `COM1..9`, `LPT1..9`) are rejected.
...
- **Activation Intent Journal (`activation.intent.json`)**: Activation operations write an intent record with fsync before switching pointers, perform atomic directory symlink/pointer switch, update metadata, and mark complete.
- **Startup Crash Reconciliation**: `ABSlotManager` startup reads the authoritative filesystem pointer, slot metadata, and activation journal to recover deterministic state after ungraceful termination at any step. Corrupted state fails safe rather than resetting silently.
- **Windows Classification**: Windows A/B activation is formally classified as `DEVELOPMENT / LIMITED` due to non-atomic directory symlink replacement semantics on Windows NTFS compared to POSIX `renameat2`.
...
- **Rollback Cryptographic Verification**: Rollback to `PREVIOUS` re-verifies manifest signature, resolves the key in the local trust store, checks revocation status, verifies exact file set match (rejecting added extra files), and re-computes SHA-256 for all slot files. Tampered files reject rollback and transition the slot to `QUARANTINED`.
...
- **Telemetry Freshness & Evidence**: Safety inputs require timestamped evidence envelopes (`observed_at`). Scalar values without timestamps and telemetry older than `max_age_seconds` or skewed into the future return `STALE_TELEMETRY` and block activation.
...
- **Heuristic Warnings**: Suspicious terms emit warnings in `ExclusionReport` without silently deleting legitimate source files (e.g. `tokenizer.py` is preserved).
...
- **Trusted Source Resolution**: Deployment requests require `release_id` and content digests; arbitrary direct `artifact_url` parameters are removed.
...
- **Mojibake Check**: Scanned all tracked `.py`, `.md`, `.json`, `.yaml`, `.yml`, `.ts`, `.tsx`, `.css`, `.html`, `.toml`, `.sh`, `.ps1` files. All legacy double-encoded sequences across historic milestone reports were repaired. Final mojibake count: **0**.
- **Control Character Scan**: Scanned all tracked files for ASCII control bytes (< 0x20) excluding `\t`, `\n`, `\r`. Final unexpected control character count: **0**.

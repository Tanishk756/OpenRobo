...
async def ingest_github_repository(request: GithubIngestionRequest, db: AsyncSession = Depends(get_db)):
    service = IngestionService(github_token=getattr(settings, "github_token", None))
    try:
        result = await service.ingest_github_repository(db=db, repository_url=request.repository_url, force_refresh=request.force_refresh)
        return result
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"An unexpected error occurred during ingestion: {e}")

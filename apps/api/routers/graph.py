...
async def create_graph_edge(payload: GraphEdgeCreate, db: AsyncSession = Depends(get_db)):
    ...
    if not valid:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail={"message": "Graph edge schema validation failed", "errors": errors}
        )
    ...
    edge = GraphEdgeModel(
        subject_id=payload.subject_id, predicate=payload.predicate, object_id=payload.object_id, properties_json=payload.properties
    )
    db.add(edge)
    await db.commit()
